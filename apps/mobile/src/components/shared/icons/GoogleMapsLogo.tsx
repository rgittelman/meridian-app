/**
 * Google Maps logo — official SVG geometry via react-native-svg.
 *
 * Paths taken directly from google-maps-svgrepo-com.svg
 * (viewBox "-55.5 0 367 367").
 *
 * Five shapes, four Google brand colors:
 *   #34A853  green  — lower-left body
 *   #FBBC04  yellow — left arc
 *   #4285F4  blue   — upper-right body
 *   #1A73E8  blue   — lower-left base
 *   #EA4335  red    — left edge
 *
 * ── Brand Asset Policy (Locked) ────────────────────────────────────────────
 * APPROVED USES:
 *   • An action row that explicitly says "Open in Google Maps"
 *   • A surface that references Google Maps by name (e.g. settings/integrations)
 *
 * DO NOT USE:
 *   • Generic location rows (use Meridian MapPin / Lucide instead)
 *   • Any surface that does not explicitly identify Google Maps
 *   • As a substitute for the Apple Maps logo or any other map provider
 *
 * NOT wired into ProviderLogo until an explicit Google Maps-branded surface
 * exists in the app. Import directly when that surface is built.
 * ───────────────────────────────────────────────────────────────────────────
 */
...
export function GoogleMapsLogo({ size = 24 }: GoogleMapsLogoProps) {
  return (
    <Svg width={size} height={size} viewBox="-55.5 0 367 367">
      {/* Green — lower-left teardrop body */}
      <Path
        fill="#34A853"
